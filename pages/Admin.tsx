                      ...
                      {editingUserId === u.id ? (
                        <>
                          ...
                          <td className="p-3">
                            ...
                            <div className="mt-2">
                              <label className="text-xs font-medium text-slate-500 dark:text-slate-400">Additional Roles</label>
                              <div className="flex flex-wrap gap-1 mt-1">
                                {Object.values(Role).filter(r => r !== u.role).map(role => {
                                  const isSelected = u.additionalRoles?.includes(role) ?? false;
                                  return (
                                    <button
                                      key={role}
                                      onClick={() => {
                                        const currentAdditional = u.additionalRoles || [];
                                        const newAdditional = isSelected
                                          ? currentAdditional.filter(r => r !== role)
                                          : [...currentAdditional, role];
                                        updateUser(u.id, { additionalRoles: newAdditional });
                                      }}
                                      className={`text-xs px-2 py-1 rounded-full border transition ${
                                        isSelected
                                          ? 'bg-blue-100 dark:bg-blue-900/30 border-blue-300 dark:border-blue-700 text-blue-700 dark:text-blue-400'
                                          : 'bg-slate-50 dark:bg-slate-700 border-slate-200 dark:border-slate-600 text-slate-500 dark:text-slate-400 hover:border-blue-300 dark:hover:border-blue-600'
                                      }`}
                                    >
                                      {isSelected ? '\u2713 ' : '+ '}{role}
                                    </button>
      ...
      {activeTab === 'brands' && (
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-bold text-slate-800 dark:text-slate-200">Brand / Manufacturer Master List</h2>
          </div>
          ...
          <div className="flex gap-3">
            <input
              ...
              className="flex-1 rounded-lg border-slate-300 dark:border-slate-600 shadow-sm border p-2.5 focus:border-blue-500 focus:ring-blue-500/20 transition bg-white dark:bg-slate-700 dark:text-slate-200"
              onKeyDown={(e) => {
            ...
            <button
              ...
              className="btn-primary text-white px-4 py-2.5 rounded-lg flex items-center gap-2 disabled:opacity-50 transition"
            >
          ...
          <div className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
            <table className="w-full">
              <thead className="bg-slate-50 dark:bg-slate-700">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wide">Brand Name</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wide">Status</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wide">Actions</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                {brands.length === 0 ? (
                      ...
                      <td className="px-4 py-3">
                        {editingBrandId === brand.id ? (
                          <input
                            ...
                            className="rounded-lg border-slate-300 dark:border-slate-600 shadow-sm border p-1.5 text-sm bg-white dark:bg-slate-700 dark:text-slate-200"
                            onKeyDown={(e) => {
                              if (e.key === 'Enter') {
                                updateBrand({ ...brand, name: editingBrandName });
                          ...
                          <span className="font-medium text-slate-800 dark:text-slate-200">{brand.name}</span>
                        )}
                      </td>
                      <td className="px-4 py-3">
                        <button
                          onClick={() => updateBrand({ ...brand, active: !brand.active })}
