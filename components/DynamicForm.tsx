        ...
        <div key={attr.id} className={`${attr.type === AttributeType.DIMENSION_BLOCK ? 'md:col-span-2' : ''}`}>
          ...
          {attr.type === AttributeType.TEXT && (
            <input
              ...
              className="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 border p-2 disabled:bg-gray-100"
          ...
          {attr.type === AttributeType.NUMERIC_UNIT && (
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 border p-2 disabled:bg-gray-100"
              />
              <select
                disabled={readOnly}
                value={values[attr.id]?.unit || (attr.units?.[0] || '')}
                onChange={(e) => handleChange(attr.id, { ...values[attr.id], unit: e.target.value })}
                className="w-24 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 border p-2 disabled:bg-gray-100"
              >
          ...
          {attr.type === AttributeType.DIMENSION_BLOCK && (
            <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg grid grid-cols-3 gap-4">
              {attr.dimensionFields?.map(field => (
                <div key={field}>
                  <label className="block text-xs font-semibold text-gray-500 uppercase">{field}</label>
                  <div className="mt-1 flex items-center">
                    <span className="text-gray-400 mr-2 text-sm">{field.charAt(0)}:</span>
                    <input
                      type="number"
                      disabled={readOnly}
                      value={values[attr.id]?.[field] || ''}
                      onChange={(e) => handleChange(attr.id, { ...values[attr.id], [field]: e.target.value })}
                      className="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 border p-2 text-sm disabled:bg-gray-100"
                    />
                  </div>
                </div>
              ))}
              <div className="col-span-3 text-xs text-gray-400 text-right">Unit: mm (Default)</div>
            </div>
          )}
